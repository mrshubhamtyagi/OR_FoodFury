...
public static class HelperFunctions
{
    ...
    public static AudioType ToAudioType(this string _key)
    {
        var arr = _key.Split('.');
        if (arr.Length < 2) return AudioType.UNKNOWN;

        if (arr[1].Equals("mp3")) return AudioType.MPEG;
        if (arr[1].Equals("wav")) return AudioType.WAV;
        if (arr[1].Equals("ogg")) return AudioType.OGGVORBIS;
    ...
    {
        Debug.Log($"Time stamp : {unixTimeStamp}");
        DateTimeOffset dateTimeOffset = DateTimeOffset.FromUnixTimeMilliseconds(unixTimeStamp + 60000);
        Debug.Log($"Date Time offset : {dateTimeOffset.LocalDateTime}");
    ...
    public static string ToJSONString(this Dictionary<string, object> _dict)
    {
        JObject _object = new();
        foreach (var item in _dict)
            _object.Add(new JProperty(item.Key, item.Value));
        return JsonConvert.SerializeObject(_object);
    }



    public static JObject ToJSONObject(this Dictionary<string, object> _dict)
    {
        JObject _object = new();
        foreach (var item in _dict)
            _object.Add(new JProperty(item.Key, item.Value));
    ...
    public static string ConvertEnumToCleanString<T>(this T enumValue) where T : Enum
    {
